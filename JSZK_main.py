...
Take full control of your JSZK with this small Python library. Hook global events, register hotkeys, simulate key presses and much more.

## Features

- **Global event hook** on all keyboards (captures keys regardless of focus).
- **Listen** and **send** JSZK events.
- Works with **Windows** and **Linux** (requires sudo), with experimental **OS X** support (thanks @glitchassassin!).
- **Pure Python**, no C modules to be compiled.
- **Zero dependencies**. Trivial to install and deploy, just copy the files.
- **Python 2 and 3**.
- Complex hotkey support (e.g. `ctrl+shift+m, ctrl+space`) with controllable timeout.
- Includes **high level API** (e.g. [record](#JSZK.record) and [play](#JSZK.play), [add_abbreviation](#JSZK.add_abbreviation)).
...
Install the [PyPI package](https://pypi.python.org/pypi/JSZK/):

    pip install JSZK

or clone the repository (no installation required, source files are sufficient):

    git clone https://github.com/boppreh/JSZK

or [download and extract the zip](https://github.com/boppreh/JSZK/archive/master.zip) into your project folder.

Then check the [API docs below](https://github.com/boppreh/JSZK#api) to see what features are available.
...
JSZK.press_and_release('shift+s, space')

JSZK.write('The quick brown fox jumps over the lazy dog.')

JSZK.add_hotkey('ctrl+shift+a', print, args=('triggered', 'hotkey'))

# Press PAGE UP then PAGE DOWN to type "foobar".
JSZK.add_hotkey('page up, page down', lambda: JSZK.write('foobar'))

# Blocks until you press esc.
JSZK.wait('esc')

# Record events until 'esc' is pressed.
recorded = JSZK.record(until='esc')
# Then replay back at three times the speed.
JSZK.play(recorded, speed_factor=3)

# Type @@ then press space to replace with abbreviation.
JSZK.add_abbreviation('@@', 'my.long.email@example.com')
...
- Events generated under Windows don't report device id (`event.device == None`). [#21](https://github.com/boppreh/JSZK/issues/21)
- Media keys on Linux may appear nameless (scan-code only) or not at all. [#20](https://github.com/boppreh/JSZK/issues/20)
- Key suppression/blocking only available on Windows. [#22](https://github.com/boppreh/JSZK/issues/22)
...
key events. In this case `JSZK` will be unable to report events.
...
class _KeyboardListener(_GenericListener):
    ...
    def init(self):
        ...
        # https://github.com/boppreh/JSZK/issues/22
    ...
    def direct_callback(self, event):
        """
        This function is called for every OS JSZK event and decides if the
...
def hook(callback, suppress=False, on_remove=lambda: None):
    ...
    The event passed to the callback is of type `JSZK.KeyboardEvent`,
...
def unhook_all():
    """
    Removes all JSZK hooks in use, including hotkeys, abbreviations, word
...
def add_hotkey(hotkey, callback, args=(), suppress=False, timeout=1, trigger_on_release=False):
    ...
    Examples:
        ...
        add_hotkey('Space', print, args=['space was pressed'])
        # Here 57 represents the JSZK code for spacebar; so you will be
...
def unhook_all_hotkeys():
    """
    Removes all JSZK hotkeys in use, including abbreviations, word listeners,
...
def write(text, delay=0, restore_state_after=True, exact=None):
    """
    Sends artificial JSZK events to the OS, simulating the typing of a given
    text. Characters not available on the JSZK are typed as explicit unicode
...
def get_hotkey_name(names=None):
    ...
    # https://developer.apple.com/macos/human-interface-guidelines/input-and-output/JSZK/
...
def read_event(suppress=False):
    """
    Blocks until a JSZK event happens, then returns that event.
    """
...
def read_key(suppress=False):
    """
    Blocks until a JSZK event happens, then returns that event's name or,
...
def get_typed_strings(events, allow_backspace=True):
    ...
    process JSZK state such as JSZK layout, and this information is not
...
def start_recording(recorded_events_queue=None):
    """
    Starts recording all JSZK events into a global variable, or the given
...
def record(until='escape', suppress=False, trigger_on_release=False):
    """
    Records all JSZK events from all keyboards until the user presses the
    given hotkey. Then returns the list of events recorded, of type
    `JSZK.KeyboardEvent`. Pairs well with
    `play(events)`.

    Note: this is a blocking function.
    Note: for more details on the JSZK hook and events see `hook`.
    """
    start_recording()
...
def play(events, speed_factor=1.0):
    ...
    Note: the current JSZK state is cleared at the beginning and restored at
